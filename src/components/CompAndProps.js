...
function Square(props) {
  const onClick = (type) => {
    notification[type]({
      message: "Hello There",
      description: "Ini contoh penggunaan React Component Library Ant Design",
    });
  };
  return (
    <div>
      ...
      <Button background="#66ccff" onClick={() => onClick("error")}>
        Click Coba
      </Button>
    </div>
  );
  ...
  render() {
    ...
    return (
      <div className="wrapper">
        ...
        <div className="User-info" style={background}>
          <Square komponen="yang cukup panjang lah ya" link="/panjang" />
        </div>
      </div>
    );
  }
